@php
    ...
    $qwHuman = function ($value) use ($qwEnumLabels) {
        if ($value === null || $value === '') return '—';
        if (is_bool($value)) return $value ? 'نعم' : 'لا';
        if (is_array($value) || is_object($value)) return json_encode($value, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
        $string = (string) $value;
        return $qwEnumLabels[$string] ?? str_replace('_', ' ', $string);
    };

    $qwBadgeClass = function ($value) {
        $value = (string) $value;
        if (in_array($value, ['approved','accepted','active','paid','passed','confirmed','complete','completed','sent','issued','delivered','dispatched','feasible'])) return 'success';
        if (in_array($value, ['rejected','cancelled','failed','inactive','blacklisted','not_feasible','overdue'])) return 'danger';
        if (in_array($value, ['pending','draft','open','on_hold','revision_requested','returned_for_correction','in_progress','scheduled','queued','high','urgent'])) return 'warning';
        return '';
    };

    $qwValue = function ($item, $field, $config = []) use ($qwHuman) {
